...
This template makes a custom title page using the information in the YAML (from the `xyz.qmd` for an single doc and from `index.qmd` for a Quarto book). The default document classes in Quarto are `scrbook` and `srcartcl`, but these templates work for other document classes too. There are some other classes in the `cls` folder (krantz, svmono, elsevier). The title pages were originally inspired from [Latex Templates](http://www.latextemplates.com/cat/title-pages#google_vignette), but templates since altered considerably.

## Audience

This is documentation is written assuming you are a LaTeX user and thus already know how to do what you want in LaTeX, but you need to figure out how to tell Quarto to do what you want. 

Non-LaTeX users, who just want to use the themes that I created, can use the `titlepage-pdf` format that is included in the extension. See the chapter on the *Using the extension*. If you are a non-LaTeX user who wants to create a custom title page or cover, skim the chapter on [LaTeX document structure](06-latex.qmd) so you know what I am talking about when I reference parts of a LaTeX document.
