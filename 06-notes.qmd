...
Instead I find it easier to include `\begin{title}....\end{title}` directly and not use `\maketitle` at all. Background: within the documentclass (required for a LaTeX document), there will be a definitian of `\maketitle`, a shortcut for a `\begin{title}....\end{title}` definition that specifies the look of the title page for that document class. Typically you'll see this at the start of a LaTeX doc:
```
\begin{document}
\maketitle
```
