...
\newcommand{\titlepageauthorblock}{
\authorstyle{$for(by-author)$$by-author.name.literal$$sep$$titlepage-theme.author-sep$$endfor$\par}
}
$endif$
...
\begin{minipage}{0.4\textwidth}
\begin{flushright}
\affiliationstyle{$for(by-author)$
$for(by-author.affiliations/first)$$by-author.affiliations.name$$sep$\\
$endfor$
$endfor$
}
\end{flushright}
